...
def get_label_weight_mask_from_hierarchy(labels, ignore_label, num_classes, num_hierarchical_classes, label_weights=1.0):
  """Gets the label weight mask.
  ...
  if isinstance(label_weights, list) and len(label_weights) != num_hierarchical_classes:
    raise ValueError(
        'Length of label_weights must be equal to num_classes if it is a list, '
        'label_weights: %s, num_classes: %d.' % (label_weights, num_classes))
  ...
  label_weights = tf.constant(label_weights, tf.float32)
  weight_mask = tf.einsum('...y,y->...',
                          tf.one_hot(labels, num_hierarchical_classes, dtype=tf.float32),
                          label_weights)
  return tf.multiply(not_ignore_mask, weight_mask)
