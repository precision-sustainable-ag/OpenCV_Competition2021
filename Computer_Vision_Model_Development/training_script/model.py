...
PROB_SUFFIX = '_prob'
SCORE_SUFFIX = '_score'
MULTI_ARGMAX_SUFFIX = '_max_score'
...
def predict_labels_multi_scale(images,
  ...
  for i, image_scale in enumerate(eval_scales):
    ...
    for output in sorted(outputs_to_scales_to_logits):
      ...
      outputs_to_scores[output].append(
          tf.expand_dims(logits, 4))
      ...
      if add_flipped_images:
        ...
        outputs_to_scores[output].append(
          tf.expand_dims(logits_reversed, 4))
  ...
  for output in sorted(outputs_to_predictions):
    ...
    scores = outputs_to_scores[output]
    #scores = tf.reduce_max(tf.concat(scores, 4), axis=4)
    scores = tf.reduce_mean(tf.concat(scores, 4), axis=4)
    
    #for beets specifically
    #scores = ((scores+10)*[0.85, 1.15, 1])-10
    
    #for cvppp4c specifically
    #scores = ((scores+10)*[1.2, 1.2, 1.3, 0.9])-10 # ((scores+10)*[1.2, 1.2, 1.3, 0.8])-10
    # for new cam:
    #scores = ((scores+10)*[0.75, 1.2, 1.4, 0.9])-10
    
    #for combined3c specifically
    #scores = ((scores+10)*[1, 1, 1.1])-10
    
    #for cvppp2c specifically
    #scores = ((scores+10)*[0.9, 1.1])-10 #((scores+10)*[0.8, 1.2])-10
    
    #for oak specifically
    #scores = ((scores+10)*[1.0, 1.0, 0.95])-10
    
    #for oak_ext specifically
    #scores = ((scores+10)*[1.0, 1.0, 1.05])-10
    
    #outputs_to_predictions[output] = tf.argmax(predictions, 3)
    outputs_to_predictions[output] = tf.argmax(scores, 3)
    outputs_to_predictions[output + PROB_SUFFIX] = tf.nn.softmax(scores)
    outputs_to_predictions[output + SCORE_SUFFIX] = scores
    outputs_to_predictions[output + MULTI_ARGMAX_SUFFIX] = tf.argmax(scores, 3)
...
def predict_labels(images, model_options, image_pyramid=None):
  ...
  for output in sorted(outputs_to_scales_to_logits):
    ...
    # for new cam:
    #logits = ((logits+10)*[0.75, 1.2, 1.4, 0.9])-10
    #logits = ((logits+10)*[1.2, 1.2, 1.3, 0.95])-10 #((logits+10)*[1.2, 1.2, 1.3, 0.8])-10
    
    #for cvppp2c specifically
    #logits = ((logits+10)*[0.9, 1.1])-10 #((logits+10)*[0.8, 1.2])-10
    
    #for oak specifically
    #logits = ((logits+10)*[1.0, 1.0, 0.95])-10 
    
    #for oak_ext specifically
    #logits = ((logits+10)*[1.0, 1.0, 1.05])-10 
    
    
    #for combined3c specifically
    #logits = ((logits+10)*[1, 1, 1.1])-10
    ...
    if model_options.prediction_with_upsampled_logits:
      ...
      predictions[output + MULTI_ARGMAX_SUFFIX] = tf.argmax(logits, 3)
      predictions[output + PROB_SUFFIX] = tf.nn.softmax(logits)
      predictions[output + SCORE_SUFFIX] = logits
    else:
      ...
      predictions[output + PROB_SUFFIX] = tf.image.resize_bilinear(
          ...
          align_corners=True,
          name='resize_prob')
      predictions[output + SCORE_SUFFIX] = tf.image.resize_bilinear(
          logits,
          tf.shape(images)[1:3],
          align_corners=True,
          name='resize_score')    
