...
def add_softmax_cross_entropy_loss_for_each_scale(scales_to_logits,
  ...
  for scale, logits in six.iteritems(scales_to_logits):
    ...
    ### Add new method for determining the appropiate weight from hierarchical label
    weights = utils.get_label_weight_mask_from_hierarchy(
        scaled_labels, ignore_label, num_classes, num_hierarchical_classes=8, label_weights=loss_weight)
    ...
    if gt_is_matting_map:
      ...
      ### Adding translation from hierarchical labels to normal labels
      #tf.print(scaled_labels, output_stream=sys.stderr)
      
      # some plant
      #condition = tf.equal(scaled_labels, tf.ones(tf.shape(scaled_labels), tf.int32)*1) # some plant
      #simple_train_labels = tf.where(condition, tf.ones(tf.shape(scaled_labels), tf.int32)*2, scaled_labels)
      
      # beets
      #condition = tf.equal(simple_train_labels, tf.ones(tf.shape(simple_train_labels), tf.int32)*32) # white clover
      #simple_train_labels = tf.where(condition, tf.ones(tf.shape(simple_train_labels), tf.int32)*1, simple_train_labels)
      
      #condition = tf.greater(simple_train_labels, tf.ones(tf.shape(simple_train_labels), tf.int32)*1) # white clover flower, white clover leaf, red clover flower, red clover leaf, unknown clover flower, unknown clover leaf..
      #simple_train_labels = tf.where(condition, tf.ones(tf.shape(simple_train_labels), tf.int32)*2, simple_train_labels)
      
      #grass clover cvppp2
      #condition = tf.equal(scaled_labels, tf.ones(tf.shape(scaled_labels), tf.int32)*3) # weeds
      #simple_train_labels = tf.where(condition, tf.ones(tf.shape(scaled_labels), tf.int32)*25, scaled_labels)
      
      #condition = tf.equal(simple_train_labels, tf.ones(tf.shape(scaled_labels), tf.int32)*2) # grass
      #simple_train_labels = tf.where(condition, tf.ones(tf.shape(scaled_labels), tf.int32)*3, simple_train_labels)
     
      
      condition = tf.equal(scaled_labels, tf.ones(tf.shape(scaled_labels), tf.int32)*3) # broadleaf
      simple_train_labels = tf.where(condition, tf.ones(tf.shape(scaled_labels), tf.int32)*2, scaled_labels)
      
      condition = tf.equal(simple_train_labels, tf.ones(tf.shape(scaled_labels), tf.int32)*4) # clover
      simple_train_labels = tf.where(condition, tf.ones(tf.shape(scaled_labels), tf.int32)*3, simple_train_labels)
      
      condition = tf.equal(simple_train_labels, tf.ones(tf.shape(scaled_labels), tf.int32)*5) # poales
      simple_train_labels = tf.where(condition, tf.ones(tf.shape(scaled_labels), tf.int32)*2, simple_train_labels)
      
      condition = tf.equal(simple_train_labels, tf.ones(tf.shape(scaled_labels), tf.int32)*6) # poales
      simple_train_labels = tf.where(condition, tf.ones(tf.shape(scaled_labels), tf.int32)*2, simple_train_labels)
      
      condition = tf.equal(simple_train_labels, tf.ones(tf.shape(scaled_labels), tf.int32)*7) # poales
      simple_train_labels = tf.where(condition, tf.ones(tf.shape(scaled_labels), tf.int32)*2, simple_train_labels)
      
      #condition = tf.equal(simple_train_labels, tf.ones(tf.shape(scaled_labels), tf.int32)*25) # weeds
      #simple_train_labels = tf.where(condition, tf.ones(tf.shape(scaled_labels), tf.int32)*4, simple_train_labels)
      
      #condition = tf.equal(simple_train_labels, tf.ones(tf.shape(scaled_labels), tf.int32)*6) # dandelion
      #simple_train_labels = tf.where(condition, tf.ones(tf.shape(scaled_labels), tf.int32)*4, simple_train_labels)
      
      #condition = tf.equal(simple_train_labels, tf.ones(tf.shape(scaled_labels), tf.int32)*7) # shepherds purse
      #simple_train_labels = tf.where(condition, tf.ones(tf.shape(scaled_labels), tf.int32)*4, simple_train_labels)
      
      #condition = tf.equal(simple_train_labels, tf.ones(tf.shape(scaled_labels), tf.int32)*8) # thistle
      #simple_train_labels = tf.where(condition, tf.ones(tf.shape(scaled_labels), tf.int32)*4, simple_train_labels)
      
      #condition = tf.equal(simple_train_labels, tf.ones(tf.shape(scaled_labels), tf.int32)*9) # white clover flower
      #simple_train_labels = tf.where(condition, tf.ones(tf.shape(scaled_labels), tf.int32)*1, simple_train_labels)
      
      #condition = tf.equal(simple_train_labels, tf.ones(tf.shape(scaled_labels), tf.int32)*10) # white clover leaf
      #simple_train_labels = tf.where(condition, tf.ones(tf.shape(scaled_labels), tf.int32)*1, simple_train_labels)
      
      #condition = tf.equal(simple_train_labels, tf.ones(tf.shape(scaled_labels), tf.int32)*11) # red clover flower
      #simple_train_labels = tf.where(condition, tf.ones(tf.shape(scaled_labels), tf.int32)*0, simple_train_labels)
      
      #condition = tf.equal(simple_train_labels, tf.ones(tf.shape(scaled_labels), tf.int32)*12) # red clover leaf
      #simple_train_labels = tf.where(condition, tf.ones(tf.shape(scaled_labels), tf.int32)*0, simple_train_labels)
      
      #condition = tf.greater(simple_train_labels, tf.ones(tf.shape(scaled_labels), tf.int32)*8) # white clover flower, white clover leaf, red clover flower, red clover leaf, unknown clover flower, unknown clover leaf..
      #simple_train_labels = tf.where(condition, tf.ones(tf.shape(scaled_labels), tf.int32)*1, simple_train_labels)
      
      #condition = tf.greater(scaled_labels, tf.ones(tf.shape(scaled_labels), tf.int32)*0) # white clover flower, white clover leaf, red clover flower, red clover leaf, unknown clover flower, unknown clover leaf..
      #simple_train_labels = tf.where(condition, tf.ones(tf.shape(scaled_labels), tf.int32)*1, scaled_labels)
      
      #tf.print(simple_train_labels, output_stream=sys.stderr)
      
      train_labels = tf.one_hot(
          simple_train_labels, num_classes, on_value=1.0, off_value=0.0)
